...
function LaboratoriosAdmin({ laboratorios, setLaboratorios, users, responsableLaboratorios, setResponsableLaboratorios, notify }) {
  ...
  const toggleResponsable = async (labId, responsableId) => {
    ...
    if (exists) {
      const { error } = await supabase.from("responsable_laboratorios").delete().match({ laboratorioId: labId, responsableId });
      ...
      const { error } = await supabase.from("responsable_laboratorios").insert({ responsableId, laboratorioId: labId });
                ...
                <tr key={a.id} style={{ borderBottom: "1px solid #f5f5f5", opacity: a.activo ? 1 : 0.55 }}>
                  ...
                  <td style={{ padding: "10px 10px" }}>
                    <div style={{ display: "flex", gap: 6 }}>
                      ...
                      {canModifyAsignatura(a) && <button onClick={() => removeAsignatura(a.id)} style={{ padding: "4px 10px", border: "1px solid #ddd", borderRadius: 6, background: "white", cursor: "pointer", fontSize: 11, fontWeight: 600, color: "#c0392b" }}>Quitar</button>}
                    </div>
                  </td>
                </tr>
              );
